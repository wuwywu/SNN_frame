...
# 该数据集是事件的集合，放电为1，不放点为0
# 输出维度为(bitch_size, polar, H, W, steps)
# steps：自己设定的步数，与SNN的窗口一致
# polar：极性，该数据集有两个0，1；事件发生都放再polar=0上
# dt是步长间的间隔 --> 所以steps*dt是数据预处理的感受野
# 这个数据是预处理后的，如翻转，旋转
class NMNIST(Dataset):
    ...
    def preprocessing(self, src_path, save_path=None):
        ...
        if save_path is not None:
            ...
            np.save(save_path+"/data.npy", self.eventflow)
            np.save(save_path+"/label.npy", self.label)


if __name__=="__main__":
    # path = r"./data/NMNIST_npy/Test"
    # nmnist = NMNIST(False, 2, 5, path)
    # src_path = r"./Test"
    # save_path = r"./data/NMNIST_npy/Test"
    # nmnist.preprocessing(src_path, save_path)
    # time_step = 5
    def prepro():
        nmnist = NMNIST(True, 30, 5)
        # src_path = r"./Test"
        # save_path = r"./data/NMNIST_npy/Test"
        src_path = r"./Train"
        save_path = r"./data/NMNIST_npy/Train"
        nmnist.preprocessing(src_path, save_path)

    def plt_im():
        # path = r"./data/NMNIST_npy/Test"
        path = r"./data/NMNIST_npy/Train"
        nmnist = NMNIST(True, 30, 5, path)
        print(nmnist.eventflow.shape)
        data = nmnist.eventflow.sum(axis=4)
        # # print(data.shape)
        i = 20130
        image_data = data[i, 0, :, :]  # 选择第一个样本、第一个极性、所有行和列、指定时间步的数据
        # print(image_data.shape)
        print(nmnist.label[i])

        # 还原图像
        image_data = image_data.reshape(34, 34)  # 调整形状为34x34
